...
from deeppixel import db, bcrypt
from deeppixel.models import User, Content, UserLibrary
from deeppixel.users.forms import (RegistrationForm, LoginForm, UpdateAccountForm, RequestResetForm, ResetPasswordForm)
from deeppixel.users.helper import save_picture, send_reset_email
...
def user_profile(username):
    ...
    library_content = db.session.query(Content).join(UserLibrary.content).order_by(Content.date_added.desc()).filter(UserLibrary.star.is_(True)).filter(Content.published.is_(True)).filter(UserLibrary.user_id==user.id).all()
    ...
    if current_user.is_authenticated:
        ...
        for i in range(len(library_content)):
    ...
    usr_content = zip(user_content, buttons_info)

    return render_template('user_profile.html', library=lib_content, user_content=usr_content, user=user)
